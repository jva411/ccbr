...
public class ItemConstructor {
    ...
    public ItemStack getItem(String line){
        ...
        Ib = new IsBuilder(is);
        for(int i=1;i<bits.length;i++) sb.append(bits[i]).append(' ');
        ...
        if(mp.find()){
            for(int i=0;i<mp.lastGroup();i++){
                ...
                if(starts(a, "name=")) Ib.setName(a.substring(5));
                    ...
                    Ib.setLore(lr);
                }else if(starts(a, "hide=")) {
                    try{
                        Ib.addFlag(ItemFlag.valueOf("HIDE_"+a.substring(5).toUpperCase()));
                    }catch(Exception ex){}
    ...
    public String getString(ItemStack is){
        ...
        for(Map.Entry<Enchantment, Integer> e:is.getEnchantments().entrySet()){
            sb.append(" [").append(e.getKey().toString().split(":")[1].split(",")[0].toUpperCase()).append('=').append(e.getValue()).append(']');
        }
        for(ItemFlag If:is.getItemFlags()) sb.append(" [hide=").append(If.toString().substring(5)).append(']');
        try{
