    ...
    public boolean onCommand(CommandSender snd, Command Cmd, String lb, String[] args) {
        ...
        if(cmd.equals("shop")){
            if(snd instanceof Player){
                if(args.length>0){
                    Player p = (Player)snd;
                    if(p.hasPermission("CCBR_Cash.shop.adm")){
                        if(args.length>3){
                            if(args[0].equalsIgnoreCase("set")){
                                Config cfg = new Config(this, args[1]);
                                if(args[3].equalsIgnoreCase("[i]")){
                                    if(IsBuilder.isAir(p.getItemInHand())) return true;
                                    cfg.set(args[2], api.Icons.getString(p.getItemInHand()));
                                    cfg.saveConfig();
                                    p.sendMessage("§aSalvo com sucesso!");
                                    return true;
                                }else if(!args[3].matches(".*?\\D+.*?")){
                                    cfg.set(args[2], Integer.parseInt(args[3]));
                                    cfg.saveConfig();
                                    p.sendMessage("§aSalvo com sucesso!");
                                    return true;
                                }
                                ...
                                if(cfg.existeConfig()){
                                    String a = cfg.getString(args[2]);
                                    if(a.length()>0) arr.add(a);
                                    for(String b:cfg.getConfig().getStringList(args[2])) arr.add(b);
                                }
                                if(args[3].equalsIgnoreCase("[i]")){
                                    if(IsBuilder.isAir(p.getItemInHand())) return true;
                                    arr.add(api.Icons.getString(p.getItemInHand()));
                                    cfg.set(args[2], arr);
                                    cfg.saveConfig();
                                    p.sendMessage("§aSalvo com sucesso!");
                                    return true;
                                ...
                                p.sendMessage("§e/shop add (file) (path) (valor <[i] - para setar o item na mão!>)");
                                p.sendMessage("§e/shop set (file) (path) (valor <[i] - para setar o item na mão!>)");
                            }
                        }else{
                            p.sendMessage("§e/shop add (file) (path) (valor <[i] - para setar o item na mão!>)");
                            p.sendMessage("§e/shop set (file) (path) (valor <[i] - para setar o item na mão!>)");
            ...
            if(snd.hasPermission("CCBR_Cash.cash.adm")){
                if(args.length>0){
                    ...
                    }else if(args[0].equalsIgnoreCase("metas")){
                        if(snd instanceof Player){
                            api.Meta.openInv((Player)snd);
                            return true;
                        }
                    }else{
